...
def warp(img, mtx, dist):  # mts, dist
    ...
    # ])
    src = np.float32([
        (33, 969),  # bottom-left corner
        (595, 512),  # top-left corner
        (1250, 512),  # top-right corner
        (1826, 969)  # bottom-right corner
    ])
    # src = np.float32([
...
def project_lane_info(img, binary_warped, ploty, left_fitx, right_fitx, M_inv, left_curverad, right_curverad, veh_pos):
    ...
    pts_left_i = np.array(
        [np.transpose(np.vstack([left_fitx+margin+150, ploty]))])
    pts_right_i = np.array(
        [np.flipud(np.transpose(np.vstack([right_fitx-margin-150, ploty])))])
    pts_i = np.hstack((pts_left_i, pts_right_i))

    # Draw the lane onto the warped blank image
    colorwarp_img = cv2.polylines(
        color_warp, np.int_([pts_left]), False, (0, 0, 255), 50)
    colorwarp_img = cv2.polylines(color_warp, np.int_(
        [pts_right]), False, (0, 0, 255), 50)
    ...
    cv2.putText(out_img, 'Curve Radius [m]: '+str((left_curverad+right_curverad)/2)[
                :7], (5, 80), cv2.FONT_HERSHEY_COMPLEX_SMALL, 1.5, (255, 255, 255), 2, cv2.LINE_AA)
    cv2.putText(out_img, 'Center Offset [m]: '+str(veh_pos)[:7], (5, 110),
...
def lane_finding_pipeline(img, init, mts, dist):
    ...
    ## checking ###
    binary_thresh_s = np.dstack(
        (binary_thresh, binary_thresh, binary_thresh))*255
    binary_warped_s = np.dstack(
    ...
    out_img, colorwarp_img, newwarp = project_lane_info(
        img, binary_warped, ploty, left_fitx, right_fitx, M_inv, left_curverad, right_curverad, veh_pos)
...
def main():
    ...
    cap = cv2.VideoCapture('./data/video/newdata/vid2.mp4')  # test_sample.mp4
