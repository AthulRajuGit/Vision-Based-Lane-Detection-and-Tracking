...
flags.DEFINE_string(
    'output', './outputs/test_sample_result.avi', 'path to output video')
...
def main(_argv):
    ...
    #####################################
    init = True
    mtx, dist = lane_findingnew.distortion_factors()
    ...
    while True:
        ...
        # Lane finding #
        ###########################
        # img_out, angle, colorwarp, draw_poly_img = lane_findingnew.lane_finding_pipeline(
        #     frame, init, mtx, dist)
        ...
        #     init = False
        image = frame
        frame1, invM = preprocess.warp(frame)
        frame = preprocess.grayscale(frame1)
        frame2 = preprocess.threshold(frame)
        frame3 = preprocess.canny(frame2,100,300)
        frame, left_curverad, right_curverad = laneDetection.search_around_poly(frame3)
        ...
        #         count), (5, 50), cv2.FONT_HERSHEY_COMPLEX_SMALL, 1.5, (0, 255, 0), 2)
        ...
        # draw bbox on screen
            ...
            ###################################
            #               car :3                bus                truck
            if class_name == "car" or class_name == "bus" or class_name == "truck" or class_name == "person":

                mid_x = (bbox[0]+bbox[2])/2
                mid_y = (bbox[1]+bbox[3])/2
                # apx_distance = round(((1 - (bbox[2]*ratio - bbox[0]*ratio))**4),1)
                apx_distance = round((((height-bbox[3]))*ratio)*4.5, 1)
                #cv2.putText(frame, '{:.2f}'.format(apx_distance), (int(mid_x),int(mid_y)), cv2.FONT_HERSHEY_SIMPLEX, 0.7, (255,255,255), 2)
                ...
                if apx_distance <= 13:
                    ...
                    # cv2.putText(frame, 'WARNING!!!', (400,150), cv2.FONT_HERSHEY_SIMPLEX, 5, (0,0,255), 4)
                    # cv2.putText(frame,  class_name + str(track.track_id), (400,220), cv2.FONT_HERSHEY_SIMPLEX, 3, (0,0,255), 3)
                    ...
                    system("say {}".format("warning warning warning"))
  
                    # # The text that you want to convert to audio
                    # mytext = 'Warning warning warning'
                    ...
                    # engineio.say('Sally sells seashells by the seashore.')
                    # engineio.say('The quick brown fox jumped over the lazy dog.')
                    # engineio.runAndWait()
        ...
        if not FLAGS.dont_show:
            ...
            cv2.namedWindow('Output Video2', cv2.WINDOW_NORMAL)
            cv2.imshow("Output Video2", frame2)
            cv2.namedWindow('Output Video3', cv2.WINDOW_NORMAL)
            cv2.imshow("Output Video3", frame3)
            cv2.namedWindow('Output Video4', cv2.WINDOW_NORMAL)
            cv2.imshow("Output Video4", frame3)
